...
describe('Group Routes', () => {
  ...
  describe('GET /api/groups/stats', () => {
    ...
    it('should handle service errors', async () => {
      mockGroupServiceInstance.getGroupStats.mockRejectedValue(new Error('Database error'));
  ...
  describe('POST /api/groups/join-random', () => {
    ...
    it('should join random group successfully', async () => {
      const mockJoinResult = {
        ...
        group: {
          ...
          maxMembers: 10,
          members: [
            { userId: 123, login: 'testuser', publicKey: 'key1', joinedAt: '2025-01-01T00:00:00.000Z' }
          ]
        }
      };
    ...
    it('should handle join failure', async () => {
      ...
      mockGroupServiceInstance.joinRandomGroup.mockResolvedValue(mockFailResult);
    ...
    it('should handle service errors', async () => {
      mockGroupServiceInstance.joinRandomGroup.mockRejectedValue(new Error('Database error'));
  ...
  describe('GET /api/groups/:groupId', () => {
    ...
    it('should get group details successfully', async () => {
      const mockGroupData = {
        ...
        members: [
          { userId: 123, login: 'testuser', publicKey: 'key1', joinedAt: '2025-01-01T00:00:00.000Z' },
          { userId: 456, login: 'otheruser', publicKey: 'key2', joinedAt: '2025-01-02T00:00:00.000Z' }
        ]
      };

      mockGroupServiceInstance.getGroupWithMembers.mockResolvedValue(mockGroupData);
      ...
      expect(mockGroupServiceInstance.getGroupWithMembers).toHaveBeenCalledWith('group-123');
    });

    it('should handle group not found', async () => {
      mockGroupServiceInstance.getGroupWithMembers.mockRejectedValue(new Error('Group not found'));
  ...
  describe('GET /api/groups/:groupId/messages', () => {
    ...
    it('should get group messages successfully', async () => {
      ...
      expect(mockGroupServiceInstance.getGroupMessages).toHaveBeenCalledWith('group-123', 50);
    });
    ...
    it('should handle custom message limit', async () => {
      ...
      expect(response.status).toBe(200);
      expect(mockGroupServiceInstance.getGroupMessages).toHaveBeenCalledWith('group-123', 20);
    });
  ...
  describe('GET /api/groups (admin route)', () => {
    it('should get active groups with pagination', async () => {
      ...
      expect(mockGroupServiceInstance.getActiveGroups).toHaveBeenCalledWith(1, 20);
    });
  });

  describe('POST /api/groups/cleanup (admin route)', () => {
    it('should cleanup inactive groups successfully', async () => {
      mockGroupServiceInstance.cleanupInactiveGroups.mockResolvedValue(undefined);
