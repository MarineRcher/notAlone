	...
	return new Promise((resolve, reject) => {
		...
		function checkProgress() {
			if (aliceConnected && bobConnected && aliceJoined && bobJoined) {
				console.log(
					"✅ [GROUP-CLEANUP] Both users joined, starting cleanup test...",
				);
		...
		function testGroupLeaving() {
			...
			setTimeout(() => {
				...
				console.log(
					"👤 [GROUP-CLEANUP] Bob leaving group (should trigger cleanup)...",
				);
				bobSocket.emit("leave_group", { groupId: testGroupId });

				setTimeout(() => {
					console.log(
						"✅ [GROUP-CLEANUP] Test completed! Check server logs for cleanup messages.",
					);
		...
		aliceSocket.on("group_members", data => {
			console.log(
				"👥 [GROUP-CLEANUP] Alice got group members:",
				data.members.length,
			);
			aliceJoined = true;
			checkProgress();
		...
		bobSocket.on("group_members", data => {
			console.log(
				"👥 [GROUP-CLEANUP] Bob got group members:",
				data.members.length,
			);
			bobJoined = true;
			checkProgress();
