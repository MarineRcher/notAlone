...
async function testNobleHttpEndpoints() {
  ...
  try {
    ...
    const protocolResponse = await axios.get(`${SERVER_URL}/api/noble-groups/test/protocol`);
    console.log('🔑 Protocol Info:', JSON.stringify(protocolResponse.data, null, 2));
    
    // Test stats endpoint
    const statsResponse = await axios.get(`${SERVER_URL}/api/noble-groups/stats`);
...
async function testNobleGroupChat() {
  ...
  try {
    ...
    aliceSocket.on('group_members', (data) => {
      console.log(`👥 Alice received group members:`, data.members.map(m => m.username));
    });
    ...
    aliceSocket.on('group_message', (data) => {
      ...
      console.log(`   Payload length: ${data.encryptedMessage.encryptedPayload?.length || 0}`);
    });
    ...
    bobSocket.on('group_members', (data) => {
      console.log(`👥 Bob received group members:`, data.members.map(m => m.username));
    });
    ...
    const mockEncryptedMessage = {
      ...
      encryptedPayload: Array.from(new TextEncoder().encode('Mock encrypted payload')),
