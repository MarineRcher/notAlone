	...
	try 
{
		...
		const recipientPublicKeyData = utils.base64ToBytes(
			sessionInitResponse.publicKey
		);
		...
		const responseSignature = utils.base64ToBytes(
			sessionInitResponse.signature
		);
		...
		if (!isSignatureValid) 
{
		...
		const encryptedMessage
			= await messageManager.encryptMessageWithKeyExchange(
				initialMessage,
				recipientId,
				userId,
				symmetricKey,
		...
		if (!sessionKeyBase64) 
{
			...
			const newSessionKeyBase64
				= await secureStorage.getSessionKey(recipientId);
...
{
		// Get the group key
		const groupKeyBase64 = await secureStorage.getItem(
			`group_key_${groupId}`
		);
	...
	try 
{
		...
		const encryptedMessage
			= messageManager.deserializeMessage(encryptedData);
		...
		if (messageManager.hasKeyExchange(encryptedMessage)) 
{
			...
			// This is simplified for the example
			const senderPublicKeyBase64 = await api.get(
				`/users/${senderId}/public-key`
			);
			const senderPublicKeyBytes = utils.base64ToBytes(
				senderPublicKeyBase64.data
			);
			...
			if (memberId !== creatorId) 
{
				...
				const publicKey
					= await keyManager.importPublicKey(publicKeyBytes);
...
{
				await sendEncryptedGroupMessage(
					stringData,
					recipientId,
					userId
				);
