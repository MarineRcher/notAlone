...
export const useEncryption = (): EncryptionContextValue => {
   ...
   */
  const handleKeyExchangeMessage = useCallback(async (
    ...
    try {
      ...
      const senderPublicKeyBytes = utils.base64ToBytes(senderPublicKey);
      const importedSenderPublicKey = await keyManager.importPublicKey(senderPublicKeyBytes);
      ...
      if (message.groupId) {
        ...
        // Store the key
        await secureStorage.setItem(`group_key_${groupId}`, sessionKeyBase64);
        ...
        const groupSession: GroupSession = {
          groupId,
          sessionId: existingGroupSession?.sessionId || `group-${groupId}`,
          symmetricKey: sessionKeyBase64,
          members: existingGroupSession?.members || [senderId, userId],
          established: true,
        ...
        setState(prevState => ({
          ...
          pendingSessions: prevState.pendingSessions.filter(id => id !== senderId)
        }));
      }
  ...
  }, [state.initialized, state.identity, state.groupSessions, state.sessions, state.pendingSessions]);
  ...
  const initialize = useCallback(async (userId: string): Promise<void> => {
    try {
      ...
      const identityKeyPair = await keyManager.getOrCreateIdentityKeyPair();
      const publicKeyData = await keyManager.exportKey(identityKeyPair.publicKey);
      ...
      for (const sessionUserId of sessionUsers) {
        const sessionKeyBase64 = await secureStorage.getSessionKey(sessionUserId);
        if (sessionKeyBase64) {
   ...
   */
  const initSession = useCallback(async (userId: string): Promise<void> => {
    try {
      if (!state.initialized || !state.identity) {
        throw new EncryptionError(
          'Encryption not initialized',
      ...
      setState(prevState => ({
        ...
        pendingSessions: prevState.pendingSessions.filter(id => id !== userId)
      }));
    } catch (error) {
  ...
  }, [state.initialized, state.identity, state.pendingSessions, state.sessions]);
  ...
  const encryptMessage = useCallback(async (
    ...
    try {
      ...
      const sessionKeyBytes = utils.base64ToBytes(session.symmetricKey);
  ...
  const decryptMessage = useCallback(async (
    ...
    try {
      ...
      if (message.header.encryptedKey && message.header.keyExchangeMode) {
        // Handle key exchange
      ...
      if (message.groupId) {
        ...
        const groupKeyBytes = utils.base64ToBytes(groupSession.symmetricKey);
        ...
        const senderPublicKeyBytes = utils.base64ToBytes(state.sessions[senderId].publicKey!);
        senderPublicKey = await keyManager.importPublicKey(senderPublicKeyBytes);
      } else {
        // Direct message
        ...
        const senderPublicKeyBytes = utils.base64ToBytes(session.publicKey);
        senderPublicKey = await keyManager.importPublicKey(senderPublicKeyBytes);
      }
  ...
  }, [state.initialized, state.identity, state.sessions, state.groupSessions, handleKeyExchangeMessage]);
   ...
   * @param groupId The ID of the group
   */
  const rotateGroupKey = useCallback(async (groupId: string): Promise<void> => {
    try {
      if (!state.initialized || !state.identity) {
        throw new EncryptionError(
          'Encryption not initialized',
   ...
   */
  const exportIdentity = useCallback(async (): Promise<{ userId: string; publicKey: string }> => {
    try {
      if (!state.initialized || !state.identity) {
        throw new EncryptionError(
          'Encryption not initialized',
  ...
  const importIdentity = useCallback(async (
    ...
    try {
      ...
      console.warn('Importing identity - this should only be used for development');
      
      // Import the private key
      const privateKeyBytes = utils.base64ToBytes(privateKey);
      const importedPrivateKey = await keyManager.importPrivateKey(privateKeyBytes);
