 ...
 * Using the robust crypto engine for reliable e2ee
 */

import { useState, useEffect, useCallback } from 'react';
import cryptoEngine, { EncryptedMessage, DecryptedMessage } from '../crypto/cryptoEngine';
...
export const useSimpleEncryption = (): EncryptionHook => {
  ...
  const initialize = useCallback(async (userId: string): Promise<void> => {
    ...
    try {
      await cryptoEngine.initialize(userId);
      setState(prev => ({ 
        ...prev, 
        initialized: true, 
        initializing: false, 
  ...
  const encryptGroupMessage = useCallback(async (
    ...
    try {
      return await cryptoEngine.encryptGroupMessage(message, groupId, senderId);
    } catch (error) {
  ...
  const decryptGroupMessage = useCallback(async (
    ...
    try {
      return await cryptoEngine.decryptGroupMessage(encryptedMessage);
    } catch (error) {
  ...
  const createGroupSession = useCallback(async (
    ...
    try {
      await cryptoEngine.createGroupSession(groupId, memberIds);
    } catch (error) {
  ...
  const getPublicKey = useCallback((): string | null => {
    return cryptoEngine.getPublicKey();
  }, []);

  // Clear all keys
  const clearKeys = useCallback(async (userId: string): Promise<void> => {
    try {
      await cryptoEngine.clearKeys(userId);
      setState({
        initialized: false,
        initializing: false,
        error: null
...
export default useSimpleEncryption; 