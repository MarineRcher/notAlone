...
import { EncryptedMessage, DecryptedMessage, KeyExchangeMessage } from '../crypto/expoCompatibleCrypto';
...
interface ExpoEncryptionHook {
  ...
  joinGroup: (groupId: string, memberIds: string[], userId: string) => Promise<KeyExchangeMessage>;
  leaveGroup: (groupId: string, userId: string) => Promise<KeyExchangeMessage>;
  processKeyExchange: (message: KeyExchangeMessage, currentUserId: string) => Promise<void>;
  refreshGroupKeys: (groupId: string, userId: string) => Promise<KeyExchangeMessage>;
}
...
export function useExpoCompatibleEncryption(): ExpoEncryptionHook {
  ...
  const initialize = useCallback(async (userId: string): Promise<void> => {
    try {
      ...
      setState({ initialized: false, initializing: false, error: errorMessage });
  ...
  const encryptGroupMessage = useCallback(async (message: string, groupId: string, senderId: string): Promise<EncryptedMessage> => {
    try {
  ...
  const decryptGroupMessage = useCallback(async (encryptedMessage: EncryptedMessage): Promise<DecryptedMessage> => {
    try {
  ...
  const createGroupSession = useCallback(async (groupId: string, memberIds: string[]): Promise<void> => {
    try {
      console.log('⚠️ createGroupSession is deprecated, use joinGroup instead');
  ...
  const joinGroup = useCallback(async (groupId: string, memberIds: string[], userId: string): Promise<KeyExchangeMessage> => {
    try {
      return await expoCompatibleCrypto.joinGroup(groupId, memberIds, userId);
    } catch (error) {
  ...
  const leaveGroup = useCallback(async (groupId: string, userId: string): Promise<KeyExchangeMessage> => {
    try {
  ...
  const processKeyExchange = useCallback(async (message: KeyExchangeMessage, currentUserId: string): Promise<void> => {
    try {
      await expoCompatibleCrypto.processKeyExchange(message, currentUserId);
    } catch (error) {
  ...
  // Clear group key
  const clearGroupKey = useCallback(async (groupId: string): Promise<void> => {
    try {
  ...
  // Refresh group keys
  const refreshGroupKeys = useCallback(async (groupId: string, userId: string): Promise<KeyExchangeMessage> => {
    try {
      return await expoCompatibleCrypto.refreshGroupKeys(groupId, userId);
    } catch (error) {
