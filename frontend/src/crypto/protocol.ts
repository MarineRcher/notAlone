...
export class NobleSignalProtocol {
	...
	static async initialize(): Promise<void> {
		console.log(
			"🔑 [NOBLE-SIGNAL] Initializing Signal Protocol with Noble cryptography...",
		);
	...
	static async sendGroupMessage(
		...
		try {
			...
			console.error(
				`🔑 [NOBLE-SIGNAL] ❌ Failed to encrypt group message: ${error}`,
			);
			throw error;
		}
	}

	static async receiveGroupMessage(
		groupId: string,
		...
		try {
			...
			if (!senderPublicKey) {
				console.warn(
					`🔑 [NOBLE-SIGNAL] ⚠️ No public key for sender ${encryptedMessage.senderId}, using dummy key`,
				);
			...
			if (!senderSession) {
				...
				senderSession = new SenderKeySession(
					groupId,
					encryptedMessage.senderId,
				);
				this.senderSessions.set(senderSessionKey, senderSession);
				console.log(
					`🔑 [NOBLE-SIGNAL] Created session for sender: ${encryptedMessage.senderId}`,
				);
			}

			if (!senderPublicKey) {
				throw new Error(
					`No public key for sender ${encryptedMessage.senderId}. Sender key distribution required.`,
				);
			}
			...
			console.error(
				`🔑 [NOBLE-SIGNAL] ❌ Failed to decrypt group message: ${error}`,
			);
			throw error;
		}
	}
	...
	static async addGroupMember(groupId: string, memberData: any): Promise<void> {
		...
		try {
			...
			if (memberData.signingPublicKey) {
				this.senderKeys.set(
					memberData.userId,
					new Uint8Array(memberData.signingPublicKey),
				);
				console.log(
					`🔑 [NOBLE-SIGNAL] Added member: ${memberData.userId} to group ${groupId}`,
				);
			}
			...
			if (memberData.chainKey && memberData.userId) {
				...
				console.log(
					`🔑 [NOBLE-SIGNAL] Updated sender session for: ${memberData.userId}`,
				);
			}
		} catch (error) {
			console.error(
				`🔑 [NOBLE-SIGNAL] ❌ Failed to add group member: ${error}`,
			);
			throw error;
		}
	}
	...
	static async removeGroupMember(
		...
		try {
			...
			console.log(
				`🔑 [NOBLE-SIGNAL] Removed member: ${userId} from group ${groupId}`,
			);
			...
			console.error(
				`🔑 [NOBLE-SIGNAL] ❌ Failed to remove group member: ${error}`,
			);
			throw error;
		}
	}
	...
	static getIdentityFingerprint(): string {
		...
		return NobleSignalCrypto.toHex(
			this.identityKeys.identityKey.publicKey.slice(0, 16),
		);
	}
	...
	static async resetGroupSession(
		groupId: string,
		userId: string,
	): Promise<void> {
		if (!this.initialized) {
		...
		try {
			...
			console.error(
				`🔑 [NOBLE-SIGNAL] ❌ Failed to reset group session: ${error}`,
			);
			throw error;
		}
	}
	...
	static async clearAll(): Promise<void> {
		try {
			...
			const signalKeys = keys.filter(
				key => key.startsWith("noble-signal-") || key.startsWith("sender-key-"),
			);
	...
	private static async getOrCreateIdentityKeys(): Promise<IdentityKeys> {
		try {
			...
			const preKeys = Array.from({ length: 10 }, () =>
				NobleSignalCrypto.generateKeyPair(),
			);
			...
			console.error(
				"🔑 [NOBLE-SIGNAL] ❌ Failed to get/create identity keys:",
				error,
			);
			throw error;
		}
