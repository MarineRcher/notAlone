...
class CryptoEngine {
  ...
  async initialize(userId: string): Promise<void> {
    try {
      ...
      const existingPrivateKey = await SecureStore.getItemAsync(`private_key_${userId}`);
      const existingPublicKey = await SecureStore.getItemAsync(`public_key_${userId}`);
      ...
      if (existingPrivateKey && existingPublicKey) {
        ...
        await SecureStore.setItemAsync(`private_key_${userId}`, this.userKeyPair.privateKey);
        await SecureStore.setItemAsync(`public_key_${userId}`, this.userKeyPair.publicKey);
  ...
  async createGroupSession(groupId: string, memberIds: string[]): Promise<void> {
    try {
      ...
      if (!groupKey) {
        ...
        await SecureStore.setItemAsync(`group_key_${groupId}`, groupKey);
  ...
  async decryptGroupMessage(encryptedMessage: EncryptedMessage): Promise<DecryptedMessage> {
    try {
      ...
      const { iv, ciphertext, tag, senderId, timestamp, groupId } = encryptedMessage;
      ...
      // Create decipher
      const decipher = QuickCrypto.createDecipherGCM('aes-256-gcm', keyBuffer);
