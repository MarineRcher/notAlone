 ...
 */
export const generateSimpleKeyPair = async (): Promise<{ publicKey: string; privateKey: string }> => {
  try {
 ...
 */
const createMockCryptoKey = (keyData: string, type: 'public' | 'private' | 'secret'): CryptoKey => {
  return {
    type,
    extractable: true,
...
export const generateAsymmetricKeyPair = async (): Promise<KeyPair> => {
  try {
    ...
    return {
      ...
      privateKey: createMockCryptoKey(simpleKeyPair.privateKey, 'private')
    };
  } catch (error) {
...
export const mockCryptoOperations = {
  ...
  async importKey(format: string, keyData: ArrayBuffer, algorithm: any, extractable: boolean, usages: string[]): Promise<CryptoKey> {
...
export const getOrCreateIdentityKeyPair = async (): Promise<KeyPair> => {
  try {
    ...
    const existingKeyPair = await secureStorage.getItem(ENCRYPTION_CONFIG.STORAGE.keys.identityKey);
 ...
 */
export const hashData = async (data: Uint8Array | string): Promise<Uint8Array> => {
  try {
    const inputString = typeof data === 'string' ? data : base64Encode(data);
