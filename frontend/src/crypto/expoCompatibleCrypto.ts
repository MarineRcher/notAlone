...
class ExpoCompatibleCrypto {
   ...
   */
  private generateGroupKey(memberPublicKeys: Map<string, string>, groupId: string, keyVersion: number): string {
    console.log('🔐 [DEBUG] Generating group key from member public keys...');
    ...
    const sortedEntries = Array.from(memberPublicKeys.entries()).sort(([a], [b]) => a.localeCompare(b));
    console.log('🔐 [DEBUG] Sorted member IDs:', sortedEntries.map(([userId]) => userId));
    
    // Concatenate all public keys in sorted order
    const concatenatedPublicKeys = sortedEntries.map(([userId, publicKey]) => publicKey).join('');
    ...
    console.log('🔐 [DEBUG] Concatenated keys length:', concatenatedPublicKeys.length);
    ...
    console.log('🔐 [DEBUG] Generated group key (first 16 chars):', base64GroupKey.substring(0, 16));
  ...
  async initialize(userId: string): Promise<void> {
    try {
      ...
      const privateKey = await SecureStore.getItemAsync(`expo_private_key_${userId}`);
      const publicKey = await SecureStore.getItemAsync(`expo_public_key_${userId}`);
      ...
      if (privateKey && publicKey) {
        ...
        // Store securely
        await SecureStore.setItemAsync(`expo_private_key_${userId}`, newPrivateKey);
        await SecureStore.setItemAsync(`expo_public_key_${userId}`, newPublicKey);
  ...
  private async loadGroupStates(userId: string): Promise<void> {
    try {
      const statesJson = await SecureStore.getItemAsync(`expo_group_states_${userId}`);
      if (statesJson) {
        ...
        console.log('✅ Loaded group states for', this.groupStates.size, 'groups');
      }
    } catch (error) {
  ...
  private async saveGroupStates(userId: string): Promise<void> {
    try {
      ...
      await SecureStore.setItemAsync(`expo_group_states_${userId}`, JSON.stringify(statesData));
   ...
   */
  private async fetchGroupMemberPublicKeys(groupId: string): Promise<Map<string, string>> {
    try {
      console.log('🔐 [DEBUG] Fetching group member public keys for:', groupId);
      ...
      console.log('🔐 [DEBUG] Fetched public keys for', memberKeys.size, 'members');
      return memberKeys;
    } catch (error) {
      console.error('❌ Failed to fetch group member public keys:', error);
   ...
   */
  async joinGroup(groupId: string, memberIds: string[], userId: string): Promise<KeyExchangeMessage> {
    try {
      ...
      const backendMemberKeys = await this.fetchGroupMemberPublicKeys(groupId);
      ...
      if (groupState.memberKeys.size > 0) {
        ...
        console.log('🔐 [DEBUG] Generated and stored group key after join');
      }
   ...
   */
  async processKeyExchange(message: KeyExchangeMessage, currentUserId: string): Promise<void> {
    try {
      ...
      if (action === 'leave') {
        ...
        console.log('🔐 [DEBUG] Added/updated public key for user:', userId);
      }
      ...
      const needsKeyRegeneration = !groupState.groupKey || action === 'leave' || 
                                   (action === 'refresh') || 
      ...
      if (needsKeyRegeneration && groupState.memberKeys.size > 0) {
        ...
        console.log('🔐 [DEBUG] Group key regenerated:', previousGroupKey !== groupState.groupKey ? '[CHANGED]' : '[SAME]');
        console.log('🔐 [DEBUG] New group key (first 16 chars):', groupState.groupKey.substring(0, 16));
        console.log('🔐 [DEBUG] Members count:', groupState.memberKeys.size);
      } else {
        console.log('🔐 [DEBUG] Keeping existing group key - no regeneration needed');
        console.log('🔐 [DEBUG] Current group key (first 16 chars):', groupState.groupKey.substring(0, 16));
      }
      ...
      if (groupState.memberKeys.size === 0) {
        ...
        console.log('🔐 [DEBUG] Cleared group key - no members remaining');
      }
   ...
   */
  async leaveGroup(groupId: string, userId: string): Promise<KeyExchangeMessage> {
    try {
  ...
  async createGroupSession(groupId: string, memberIds: string[]): Promise<void> {
    try {
      ...
      if (!this.initialized) {
        console.error('❌ [DEBUG] Crypto not initialized for group session creation');
        throw new Error('Crypto not initialized');
      }
      ...
      console.log('🔐 [DEBUG] Existing group key in memory:', groupKey ? '[EXISTS]' : '[NOT FOUND]');
      
      if (!groupKey) {
        // Try to load from secure storage first
        const storageKey = `expo_group_key_${groupId}`;
        console.log('🔐 [DEBUG] Checking storage for existing key:', storageKey);
        const storedGroupKey = await SecureStore.getItemAsync(storageKey);
        console.log('🔐 [DEBUG] Key found in storage:', storedGroupKey ? '[EXISTS]' : '[NOT FOUND]');
        ...
        if (storedGroupKey) {
          ...
          console.log('🔐 [DEBUG] Existing key first 16 chars:', groupKey.substring(0, 16));
        } else {
          ...
          console.log('🔐 [DEBUG] Generating deterministic group key...');
          groupKey = await this.generateDeterministicGroupKey(groupId);
          console.log('🔐 [DEBUG] Generated deterministic key length:', groupKey.length);
          console.log('🔐 [DEBUG] Generated deterministic key first 16 chars:', groupKey.substring(0, 16));
          ...
          console.log('🔐 [DEBUG] Storing deterministic key with storage key:', storageKey);
          await SecureStore.setItemAsync(storageKey, groupKey);
          
          // Verify storage
          const verifyStored = await SecureStore.getItemAsync(storageKey);
          console.log('🔐 [DEBUG] Storage verification:', verifyStored ? '[SUCCESS]' : '[FAILED]');
          console.log('🔐 [DEBUG] Stored key matches generated:', verifyStored === groupKey);
          
          console.log('✅ Generated deterministic group key for group:', groupId);
        }
      } else {
        console.log('✅ Using existing group key from memory for group:', groupId);
        console.log('🔐 [DEBUG] Memory key first 16 chars:', groupKey.substring(0, 16));
      }
    } catch (error) {
      console.error('❌ [DEBUG] Failed to create group session - Error details:', error);
   ...
   */
  private async generateDeterministicGroupKey(groupId: string): Promise<string> {
    try {
      console.log('🔐 [DEBUG] Creating deterministic key for group:', groupId);
      ...
      console.log('🔐 [DEBUG] Seed string for key generation:', seedString);
      ...
      console.error('❌ [DEBUG] Failed to generate deterministic key:', error);
   ...
   */
  private async createMessageKey(messageLength: number, timestamp: number, groupKey: string, senderKey: string): Promise<string> {
  ...
  private async simpleEncrypt(plaintext: string, groupKey: string, senderKey: string, timestamp: number): Promise<{ ciphertext: string; iv: string; messageKeyHash: string; timestamp: number }> {
    try {
      // Generate random IV
      ...
      const messageKey = await this.createMessageKey(plaintext.length, timestamp, groupKey, senderKey);
      ...
      for (let i = 0; i < plaintextBytes.length; i++) {
        ciphertextBytes[i] = plaintextBytes[i] ^ messageKeyBytes[i % messageKeyBytes.length];
      }
   ...
   */
  private async simpleDecrypt(ciphertext: string, groupKey: string, senderKey: string, iv: string, messageLength: number, timestamp: number): Promise<string> {
    try {
      // Convert inputs to bytes
      const ciphertextBytes = this.base64ToBytes(ciphertext);
      
      // Recreate message-specific key (same as encryption)
      const messageKey = await this.createMessageKey(messageLength, timestamp, groupKey, senderKey);
      const messageKeyBytes = this.hexToBytes(messageKey);
      
      // Decrypt using XOR with message-specific key
      const plaintextBytes = new Uint8Array(ciphertextBytes.length);
      for (let i = 0; i < ciphertextBytes.length; i++) {
        plaintextBytes[i] = ciphertextBytes[i] ^ messageKeyBytes[i % messageKeyBytes.length];
      }
  ...
  async encryptGroupMessage(message: string, groupId: string, senderId: string): Promise<EncryptedMessage> {
    try {
      ...
      if (!this.initialized) {
        console.error('❌ [DEBUG] Crypto not initialized for encryption');
        throw new Error('Crypto not initialized');
      }
      ...
      let groupKey = this.groupStates.get(groupId)?.groupKey;
      console.log('🔐 [DEBUG] Group key from memory:', groupKey ? '[EXISTS]' : '[NOT FOUND]');
      
      if (!groupKey) {
        // Try to load from secure storage
        const storedKey = await SecureStore.getItemAsync(`expo_group_key_${groupId}`);
        console.log('🔐 [DEBUG] Group key from storage:', storedKey ? '[EXISTS]' : '[NOT FOUND]');
        ...
        if (storedKey) {
          ...
          console.log('🔐 [DEBUG] Group key loaded from storage and cached');
        } else {
          console.error('❌ [DEBUG] No group key found for group:', groupId);
          throw new Error(`No group key found for group: ${groupId}`);
        }
      }

      console.log('🔐 [DEBUG] Group key length:', groupKey.length);
      console.log('🔐 [DEBUG] Group key first 16 chars:', groupKey.substring(0, 16));

      // Get sender's actual public key for message encryption
      const senderPublicKey = this.userKeyPair?.publicKey || senderId;
      console.log('🔐 [DEBUG] Using sender public key for encryption:', senderPublicKey.substring(0, 16));
      
      // Encrypt the message
      const timestamp = Date.now();
      console.log('🔐 [DEBUG] Starting simpleEncrypt...');
      const { ciphertext, iv, messageKeyHash } = await this.simpleEncrypt(message, groupKey, senderPublicKey, timestamp);
      ...
      console.error('❌ [DEBUG] Failed to encrypt message - Error details:', error);
      console.error('❌ [DEBUG] Stack trace:', error instanceof Error ? error.stack : 'No stack trace available');
  ...
  async decryptGroupMessage(encryptedMessage: EncryptedMessage): Promise<DecryptedMessage> {
    try {
      ...
      if (!this.initialized) {
        console.error('❌ [DEBUG] Crypto not initialized for decryption');
        throw new Error('Crypto not initialized');
      }
      ...
      if (!groupId) {
        console.error('❌ [DEBUG] Group ID missing from encrypted message');
      ...
      if (!groupKey) {
        ...
        if (storedKey) {
          ...
          console.log('🔓 [DEBUG] Group key loaded and cached for decryption');
        } else {
          console.error('❌ [DEBUG] No group key found for decryption of group:', groupId);
          throw new Error(`No group key found for group: ${groupId}`);
        }
      }
      ...
      console.log('🔓 [DEBUG] Using groupKey + iv for hash:', (groupKey + iv).substring(0, 30) + '...');
      ...
      console.log('🔓 [DEBUG] Expected truncated hash:', expectedKeyHash.substring(0, 16));
      console.log('🔓 [DEBUG] Received hash:', keyHash);
      console.log('🔓 [DEBUG] Hashes match:', expectedKeyHash.substring(0, 16) === keyHash);
      
      if (expectedKeyHash.substring(0, 16) !== keyHash) {
        console.warn('⚠️ Key hash mismatch - message may be corrupted or from wrong key');
        console.warn('⚠️ [DEBUG] This suggests the sender used a different key or derivation method');
      ...
      const groupState = this.groupStates.get(groupId!);
      const senderPublicKey = groupState?.memberKeys.get(senderId) || this.userKeyPair?.publicKey || senderId;
      const messageLength = this.base64ToBytes(ciphertext).length; // Calculate from ciphertext
      
      console.log('🔓 [DEBUG] Using sender public key:', senderPublicKey.substring(0, 16));
      
      // Decrypt the message
      console.log('🔓 [DEBUG] Starting simpleDecrypt...');
      const plaintext = await this.simpleDecrypt(ciphertext, groupKey, senderPublicKey, iv, messageLength, timestamp);
      console.log('🔓 [DEBUG] Decryption complete. Plaintext:', plaintext);
      ...
      console.error('❌ [DEBUG] Failed to decrypt message - Error details:', error);
      console.error('❌ [DEBUG] Stack trace:', error instanceof Error ? error.stack : 'No stack trace available');
   ...
   */
  async refreshGroupKeys(groupId: string, userId: string): Promise<KeyExchangeMessage> {
    try {
      ...
      console.log('🔄 [DEBUG] Generated new personal key (first 16 chars):', newPersonalKey.substring(0, 16));
      ...
      if (groupState.memberKeys.size > 0) {
        ...
        const storageKey = `expo_group_key_${groupId}`;
        await SecureStore.setItemAsync(storageKey, groupState.groupKey);
        console.log('🔄 [DEBUG] Regenerated and stored new group key after refresh');
      }
