...
type Props = NativeStackScreenProps<any, "TreeShop">;
const TreeShopScreen = ({ navigation }: Props) => {
	const [trees, setTrees] = useState<TreeItem[] | null>(null);
	const [loading, setLoading] = useState(true);
	const [activeTab, setActiveTab] = useState<"trees" | "flowers">("trees");

	useEffect(() => {
		const fetchForest = async () => {
			try {
				const data = await gamesService.getTrees();
				setTrees(data);
			} catch (error) {
