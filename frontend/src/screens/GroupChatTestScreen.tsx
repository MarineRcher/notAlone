...
{
				console.log(
					"⚠️ Cannot refresh keys - encryption not initialized or no group ID"
				);
				return;
			}
				...
				try 
{
					...
					const keyExchangeMessage = await joinGroup(
						data.group.id,
						memberIds,
						userId
					);
					...
					console.error(
						"❌ Failed to setup group encryption session:",
						error
					);
				}
			}
				...
				if (data.encryptedContent && initialized) 
{
					...
					const encryptedData: any = JSON.parse(
...
{
						console.log(
							"🔓 Decrypting message for group:",
							encryptedData.groupId
						);
						const encryptedMessage: EncryptedMessage
							= encryptedData;
						const decryptedMessage
							= await decryptGroupMessage(encryptedMessage);
						...
						console.warn(
							"⚠️ Message format not compatible with ExpoCompatibleCrypto. Attempting to display raw content..."
						);
						...
						if (encryptedData.header && encryptedData.ciphertext) 
{
					...
					messageContent
						= data.encryptedContent || data.content || "[No content]";
...
{
						console.log(
							"🔄 Responding to new user join with our key..."
						);
						const ourKeyExchange = await refreshGroupKeys(
							currentGroup.id,
							userId
						);
		...
		if (!isAuthenticated) 
{
			Alert.alert(
				...
				"Please log in to use group chat",
		...
		try 
{
			...
			try
{
				const encryptedMessage = await encryptGroupMessage(
					newMessage,
					currentGroup.id,
			...
			<SafeAreaView style={styles.container}>
				...
				<View style={styles.authContainer}>
					...
					<Text style={styles.authSubtitle}>
						You need to be authenticated to access group chat
						features.
					</Text>
					<TouchableOpacity
				...
				<View style={styles.headerInfo}>
					...
					{currentGroup && (
						<Text style={styles.groupInfo}>
							{currentGroup.name} ({currentGroup.currentMembers}/
						...
						onPress={async () => 
{
							...
							try 
{
								const leaveMessage = await leaveGroup(
									currentGroup.id,
									userId
								);
				...
				{!currentGroup ? (
					<View style={styles.joinContainer}>
						...
						<Text style={styles.joinSubtitle}>
							Connect with others in real-time group conversations
							...
							<View style={styles.messageInputWrapper}>
								...
								{!initialized && (
									<View style={styles.encryptionWarning}>
										<Text
											style={styles.encryptionWarningText}
										>
						...
						try 
{
							...
							console.log(
								"✅ Group key cleared - will regenerate deterministically"
							);
							Alert.alert(
								"Success",
								"Group key cleared! It will regenerate deterministically when you send/receive a message."
							);
						} catch (error) {
							console.error(
								"❌ Failed to clear group key:",
								error
							);
